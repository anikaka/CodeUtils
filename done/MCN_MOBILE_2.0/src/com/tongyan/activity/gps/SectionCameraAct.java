...
import com.tongyan.utils.HttpDownloader;
 ...
 * @author Rubert
 * @date 2014-7-30 PM 02:26:57 
 * @version V1.9.6 
 * @Description: 工程订阅拍照
 */
public class SectionCameraAct extends AbstructCommonActivity{
	...
	private String originCameraImgPath;
 	...
 	private ArrayList<HashMap<String, String>> mRemoteImageList = null;
	...
	OnClickListener mOpenCameraListener = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			String mLocalPath = FileUtils.getSDCardPath() + Constansts.CN_CAMERA_GPS_PATH + "/" + subId + "/";
	...
	private void getPictureList(final String subId) {
		showInitDialog();
		new Thread(new Runnable() {
			@Override
			public void run() {
				String paramsStr = "{SubId:'" + subId + "',MeansType:'',DocumentName:'',StartTime:'',EndTime:'',Sort:''}";
				try {
					//GetListResponse{GetListResult={"s":null,"v":null}; }
					String jsonStr = WebServiceUtils.getRequestStr(mLocalUser.getUsername(), mLocalUser.getPassword(), String.valueOf(100), String.valueOf(1), Constansts.TYPE_OF_GETLIST_GETATTACHMENTSLISTBYPAGE, paramsStr, Constansts.METHOD_OF_GETLIST, mContext);
					mRemoteImageList = new Str2Json().getSectionImageList(jsonStr);
	...
	public void reloadListView() {
		if(mImageList != null) {
			...
			if(mRemoteImageList != null){
				mImageList.addAll(mRemoteImageList);
			}
	...
	//private final static int MES_TYPE_5 = 0xAAAA2;
