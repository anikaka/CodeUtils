...
/**
 * 
 * @className OaEmailAct
 * @author wanghb
 * @date 2014-7-14 AM 08:19:54
 * @Desc 邮件通知
 */
public class OaEmailAct extends FinalActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.oa_email);
		mTitleName.setText(getResources().getString(R.string.email_notice));//设置标题
	}
	...
	public boolean verification(){
		mTitleNameText = mTitleName.getText().toString();
		if("".equals(mTitleNameText.trim())) {
			Toast.makeText(mContext, "请填写邮件标题", Toast.LENGTH_SHORT).show();
			return false;
		}
		mMessageContentText = mMessageContent.getText().toString();
		if("".equals(mMessageContentText.trim())) {
			Toast.makeText(mContext, "请填写邮件内容", Toast.LENGTH_SHORT).show();
			return false;
		}
			...
			public void run() {
				...
				if(mCacheUserList != null) {
					for(int i = 0; i < mCacheUserList.size(); i ++) {
						...
						if(m != null) {
							String mail = m.get("UserEmail");
							if("null".equals(mail) || "".equals(mail)) {
								continue;//TODO 没有邮箱地址的就没有提示
							}
				...
				properties.put("title", mTitleNameText);
				properties.put("mailBody", mMessageContentText);
				try {
					String json = WebServiceUtils.requestM(properties, Constants.WEBSERVICE_OF_SENDMAIL, mContext);
					if(json != null) {
	...
	protected void handleOtherMessage(int index) {
		...
		case Constants.COMMON_MESSAGE_1:
			closeDialog();
			Toast.makeText(mContext, "所选人员邮箱地址均为空", Toast.LENGTH_SHORT).show();
			break;
		}
	};
